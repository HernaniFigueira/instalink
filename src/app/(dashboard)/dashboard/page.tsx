  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-bold uppercase tracking-wider text-zinc-500">Visão geral</h2>
      ...
      <div className="grid lg:grid-cols-3 gap-3 mb-6">
        <Card className="p-5">
          ...
          <p className="text-xs text-zinc-500 mt-2">{revenue.orders} pedido(s) · tíquete médio {money(revenue.ticket)}</p>
        </Card>
        <Card className="p-5">
          <p className="text-xs font-bold uppercase tracking-wider text-zinc-400 mb-3">Próximos agendamentos</p>
          {upcoming.length === 0 ? (
            ...
            <ul className="space-y-2 text-sm">
              {upcoming.map((b) => (
                <li key={b.id} className="flex items-center justify-between gap-2">
                  <span className="truncate"><strong>{b.customerName}</strong> <span className="text-zinc-500">· {b.service}{b.professional ? ` com ${b.professional}` : ''}</span></span>
                  <span className="text-xs font-bold text-zinc-500 shrink-0">{humanDay(b.date)} {b.time}</span>
                </li>
              ))}
            </ul>
          )}
          <Link href={`/agenda${q}`} className="inline-block mt-3 text-xs font-bold text-emerald-700 hover:underline">Ver agenda →</Link>
        </Card>
        <Card className="p-5">
          ...
          <Link href={`/resultados${q}`} className="inline-block mt-3 text-xs font-bold text-emerald-700 hover:underline">Ver resultados →</Link>
        </Card>
      </div>

      <h2 className="text-sm font-bold uppercase tracking-wider text-zinc-500 mb-3">Operação</h2>
