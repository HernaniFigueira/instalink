...
export default function OrganizationPage() {
  ...
  return <div className="space-y-5">
    <header className="flex items-end justify-between border-b border-zinc-200 pb-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-zinc-400">Organização</p><h1 className="text-xl font-semibold text-zinc-900 mt-1">{org.name}</h1>{orgs.length > 1 && <select aria-label="Selecionar organização" value={org.id} onChange={e=>window.location.assign(`/organizacao?organization=${e.target.value}`)} className="mt-2 border border-zinc-200 rounded-md px-2 py-1 text-xs bg-white">{orgs.map(o=><option key={o.id} value={o.id}>{o.name}</option>)}</select>}<p className="text-sm text-zinc-500 mt-1">Visão consolidada das unidades às quais você tem acesso.</p></div>{org.canManage && <button onClick={()=>setAdding(!adding)} className="text-xs font-semibold bg-zinc-900 text-white px-3 py-2 rounded-md">+ Adicionar unidade</button>}</header>
