...
/**
 * Mutação PURA do perfil da unidade — ÚNICA fonte da regra, usada pelos DOIS
 * motores: documento (updateDB) e relacional (fatia businesses+pages dentro
 * da transação). Recebe o "doc" só para achar a unidade e a página (blocos).
 */
function applyBusinessProfilePatch(d: Pick<any, 'businesses' | 'pages'>, businessId: string, body: Record<string, any>, ctx: any): void {
  ...
  if (body.features !== undefined && body.features && typeof body.features === 'object') {
    const features = normalizeFeatures(b, d.pages.find((p: any) => p.businessId === b.id)?.blocks || []);
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // MODO RELACIONAL: a MESMA mutação pura, sobre a fatia mínima
    // (businesses + pages da unidade) dentro da transação com advisory lock.
    if (relationalActive()) {
      await runRelationalWrite(
        params.id,
        (d) => applyBusinessProfilePatch(d as any, params.id, body, ctx),
        { load: { businesses: {}, pages: {} } },
      );
      return NextResponse.json({ ok: true });
    }
    await updateDB((d) => applyBusinessProfilePatch(d, params.id, body, ctx));
    return NextResponse.json({ ok: true });
  } catch {
