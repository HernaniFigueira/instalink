...
// SupportSession — reutiliza o mecanismo existente.
// Master escolhe Organization → unidade → view|admin.
...
export async function POST(req: NextRequest) {
  ...
  await updateDB((d) => {
    ...
    pushAudit(d, {
      ...
      businessId,
      supportSessionId: id,
      meta: { reason, organizationId: business.organizationId || '', via: 'master' },
    });
  });
...
export async function DELETE(req: NextRequest) {
  ...
  res.cookies.set(SUPPORT_COOKIE, '', {
    httpOnly: true, sameSite: 'none', secure: true, partitioned: true, path: '/', maxAge: 0,
  });
  return res;
}
