...
/** View PURA (DOIS motores): todo o payload da Dashboard a partir de um doc
 * mínimo da unidade. Recebe ctx do guard e o período já parseado; nenhum
 * acesso a banco aqui dentro — as regras são idênticas nos dois motores. */
function overviewPayload(db: any, ctx: any, period: number) {
  const business = ctx.business;
  // Financeiro só aparece para quem tem a permissão (e some do payload).
  const showMoney = can(ctx, 'financeiro');

  const bId = business.id;
  const q = `?b=${bId}`;
  const events = db.events.filter((e: any) => e.businessId === bId);
  const orders = db.orders.filter((o: any) => o.businessId === bId);
  const leads = db.leads.filter((l: any) => l.businessId === bId);
  ...
  const professionalScope = ctx.professionalScope;
  const allBookings = db.bookings.filter((b: any) => b.businessId === bId);
  const bookings = scopeBookings(allBookings, professionalScope) as any[];
  ...
  const servicesById = Object.fromEntries(
    db.services.filter((s: any) => s.businessId === bId).map((s: any) => [s.id, s]),
  );
  ...
  const orderRevenueResult = orderRevenue(
    orders.map((o: any) => ({ status: o.status, createdAt: o.createdAt, total: o.total })),
    periodWindow,
    prevWindow,
  );
  ...
  const pros = new Map(db.professionals.filter((p: any) => p.businessId === bId).map((p: any) => [p.id, p.name]));
  const upcoming = m.bookings
    ? bookings
      .filter((b: any) => ['pending', 'confirmed'].includes(b.status) && b.date >= today)
  ...
  const vids = new Set(events.filter((e: any) => e.type === 'page_view' && e.meta?.vid).map((e: any) => String(e.meta.vid)));
  ...
  const closures = m.bookings
    ? pendingClosures(bookings, servicesById, today, nowHM()).map((b: any) => ({
      id: b.id, customerName: b.customerName, date: b.date, time: b.time, status: b.status,
      service: serviceNames.get(b.serviceId) || 'Serviço',
    }))
    : [];
  ...
  const contacts = db.contacts.filter((c: any) => c.businessId === bId);
  const crm = {
    contacts: contacts.length,
    newContacts: contacts.filter((c: any) => (c.createdAt || '').slice(0, 10) >= from).length,
    registered: contacts.filter((c: any) => !!c.customerId).length,
    withConsent: contacts.filter((c: any) => c.marketingOptIn === true).length,
    leads: leads.length,
    leadsNew: leads.filter((l: any) => l.status === 'new').length,
    // Converteu = cliente com histórico de agenda/pedido.
    customers: contacts.filter((c: any) => !!c.customerId).length,
  };
  ...
  const canTasks = TASKS_PERMS.some((p: any) => can(ctx, p));
  ...
  const links = dashboardLinks(ctx.permissions);
  ...
  const queueWaitingNow = (db.queue || []).filter((q: any) =>
  ...
  const pageStats = {
    views: events.filter((e: any) => e.type === 'page_view' && e.createdAt.slice(0, 10) >= from).length,
    clicks: events.filter((e: any) => (e.type === 'button_click' || e.type === 'whatsapp_click') && e.createdAt.slice(0, 10) >= from).length,
    ...
    bookings: allBookings.filter((b: any) => (b.createdAt || '').slice(0, 10) >= from).length,
    conversions: events.filter((e: any) => e.type === 'conversion' && e.createdAt.slice(0, 10) >= from).length,
    published: !!business.published,
    slug: business.slug,
  };

  // ── WhatsApp: estado honesto (nunca "conectado" de mentira) ──
  const conversations = db.conversations.filter((c: any) => c.businessId === bId);
  const whatsapp = m.whatsapp || m.agent
    ? {
      ...
      pendingMessages: db.messages.filter((x: any) => x.businessId === bId && x.status === 'pending').length,
  ...
  const ordersPanel = m.orders
    ? {
      ...
      open: orders.filter((o: any) => !['completed', 'cancelled'].includes(o.status)).length,
      inWindow: orders.filter((o: any) => o.createdAt.slice(0, 10) >= from && o.createdAt.slice(0, 10) <= today).length,
    }
    : null;

  const productsPanel = m.products
    ? {
      total: db.products.filter((p: any) => p.businessId === bId).length,
      active: db.products.filter((p: any) => p.businessId === bId && p.active).length,
    }
    : null;
  ...
  const setupItems = setupChecklist({
    ...
    modules: m,
    counts: {
      services: db.services.filter((s: any) => s.businessId === bId && s.active).length,
      availability: db.availability.filter((a: any) => a.businessId === bId).length,
      professionals: db.professionals.filter((p: any) => p.businessId === bId && p.active).length,
      products: db.products.filter((p: any) => p.businessId === bId && p.active).length,
    },
  });
  const checklist = setupItems.map((c: any) => ({ done: c.done, label: c.label, href: `${c.href}${q}` }));
  const pendingSetup = checklist.filter((c: any) => !c.done).length;
  ...
  const resultsBlock = can(ctx, 'financeiro')
    ? (() => {
  ...
  return {
    ...
    scope: scopeInfo(ctx),
    totals: {
      visitors: events.filter((e: any) => e.type === 'page_view').length,
      uniqueVisitors: vids.size,
      clicks: events.filter((e: any) => e.type === 'button_click' || e.type === 'whatsapp_click').length,
      leads: leads.length,
      leadsNew: leads.filter((l: any) => l.status === 'new').length,
      ...
      conversions: events.filter((e: any) => e.type === 'conversion').length,
      newOrders: m.orders ? orders.filter((o: any) => o.status === 'new').length : 0,
      pendingBookings: m.bookings ? bookings.filter((b: any) => b.status === 'pending').length : 0,
    },
    ...
    recent: {
      orders: activity.orders
        ? [...orders].reverse().slice(0, 3).map((o: any) => ({ id: o.id, code: o.code, customerName: o.customerName, status: o.status, createdAt: o.createdAt }))
        : [],
      bookings: activity.bookings
        ? [...bookings].reverse().slice(0, 3).map((b: any) => ({ id: b.id, customerName: b.customerName, date: b.date, time: b.time, status: b.status }))
        : [],
      leads: activity.leads
        ? [...leads].reverse().slice(0, 3).map((l: any) => ({ id: l.id, name: l.name, phone: l.phone, origin: l.origin, status: l.status }))
        : [],
    },
  };
}
...
export async function GET(req: NextRequest) {
  ...
  if (relationalActive()) {
    // Fatia DIRECIONADA da Dashboard: as coleções que o painel AGREGA, todas
    // da unidade (a tela é uma agregação por definição — nada de outra
    // unidade/tenant entra). Membros + usuários só para nomes de responsáveis
    // nas tarefas; a janela de período é aplicada dentro da fn pura (mesmas
    // regras/contas do legado — totais são "desde sempre" da unidade).
    const db = await runRelationalRead(businessId, {
      events: {}, orders: {}, leads: {}, bookings: {}, services: {},
      professionals: {}, contacts: {}, queue: {}, conversations: {}, messages: {},
      products: {}, availability: {}, tasks: {}, members: {},
      users: (partial) => {
    ...
    return NextResponse.json(overviewPayload(db, guard.ctx, period));
  }
  return NextResponse.json(overviewPayload(guard.db, guard.ctx, period));
}
