...
export async function processWhatsappInbound(input: NormalizedWhatsappInbound) {
  ...
  const newOutboundMessageIds = await updateDB((d) => {
    ...
    if (!business?.whatsappIntegration || (business.whatsappIntegration.provider || 'meta_cloud') !== provider) return [];
    const b = business;
    ...
    const messages = [input];
        ...
        for (const m of messages) {
          ...
          const automationEnabled = agentActive(business, activeAgent) && activeAgent.channels?.whatsapp;
          if (wantsHuman) {
            conv.mode = 'human';
    ...
    return newOutboundMessageIds;
  });
  for (const id of newOutboundMessageIds) await deliverWhatsappMessage(businessId, id);
  return { outbound: newOutboundMessageIds.length };
}
